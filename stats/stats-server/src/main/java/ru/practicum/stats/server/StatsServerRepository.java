...
public interface StatsServerRepository extends JpaRepository<Event, Long> {
    @Query("select new ru.practicum.stats.dto.EventOutputDto(count(e.uri), e.app, e.uri) from Event e " +
            "where e.eventCreated between ?1 and ?2 " +
            "group by e.app, e.uri")
    List<EventOutputDto> getEvents(LocalDateTime start, LocalDateTime end);

    @Query("select new ru.practicum.stats.dto.EventOutputDto(count(e.uri), e.app, e.uri) from Event e " +
            "where e.eventCreated between ?1 and ?2 and lower(e.uri) like lower(concat('%', ?3, '%')) " +
            "group by e.app, e.uri")
